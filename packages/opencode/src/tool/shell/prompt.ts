...
function bashCommandSection(chain: string, limits: Limits, defaultTimeoutMs: number) {
  ...
  - Optional timeout in milliseconds; if not set, commands time out after ${defaultTimeoutMs}ms.
  - If output exceeds ${limits.maxLines} lines or ${limits.maxBytes} bytes it is truncated and the full output is written to a file; inspect it with \`cat\`, \`sed -n\`, or \`grep\`.
  - Quote file paths containing spaces with double quotes.
  - When commands are independent, issue multiple bash calls in a single message to run them in parallel.
  - ${chain}
  - Use ';' only for sequential commands whose later steps should not depend on earlier success.
  - Do not separate commands with newlines (newlines are fine inside quoted strings).`
}
